...
add_person:-
   write('Person to add Gender-Person'),nl,
   read(Gender-Person),
...
add_parents(Person):-
   write('Add parents'),nl,
   read(Father-Mother),
   \+ parent(Father, Person), \+ parent(Mother, Person),
   assert(parent(Father, Person)), assert(parent(Father, Person)).
